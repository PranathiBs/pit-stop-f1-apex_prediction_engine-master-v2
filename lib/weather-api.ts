...
const PYTHON_BACKEND = process.env.NEXT_PUBLIC_PYTHON_BACKEND_URL || 'http://localhost:8888';
...
export async function getCurrentWeather(lat: string, lon: string, context?: { year: number, gp: string, session: string }): Promise<WeatherData | null> {
    try {
        // If context is provided, try the Gold Standard backend cache first
        if (context) {
            const f1Weather = await getF1Weather(context.year, context.gp, context.session);
            if (f1Weather && f1Weather.status !== 'Data Pending') {
                return {
                    temp: f1Weather.air_temp,
                    feels_like: f1Weather.air_temp + 2,
                    humidity: 50,
                    wind_speed: 5,
                    wind_deg: 0,
                    description: f1Weather.status.toLowerCase(),
                    icon: f1Weather.status === 'Rain' ? '10d' : '01d',
                    main: f1Weather.status,
                    rain_probability: f1Weather.rain_prob * 100,
                    clouds: f1Weather.status === 'Clear' ? 10 : 50,
                    visibility: 10000
                };
            }
        }
...
// Get backend-cached FastF1 weather
export async function getF1Weather(year: number, gp: string, session: string = 'Race') {
    try {
        const response = await fetch(`${PYTHON_BACKEND}/api/weather/${year}/${gp}/${session}`);
