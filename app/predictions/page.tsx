                ...
                {selectedCircuit && (
                    <>
                        ...
                        <div className={styles.mlSection}>
                            ...
                            {!mlLoading && mlResult?.error && (
                                <div className={styles.mlOffline}>
                                    <p>{mlResult.error}</p>
                                    {mlResult.details && <p style={{ fontSize: '0.7rem', opacity: 0.6, marginTop: '4px' }}>Details: {mlResult.details}</p>}
                                    <br />
                            ...
                            {!mlLoading && mlResult && !mlResult.error && mlResult.predictions.length > 0 && (
                                <>
                                    <PredictionVisualizer data={mlResult.predictions} />
                                    <div className={styles.mlGrid}>
                                        {mlResult.predictions.slice(0, 10).map((p, i) => {
                                            const prevDriver = i > 0 ? mlResult.predictions[i-1] : null;
                                            const intervalGap = prevDriver ? p.base_pace - prevDriver.base_pace : null;
                                            
                                            return (
                                                <div key={p.driver} className="bg-[#0b0c0d] border border-gray-800 rounded-lg p-3 mb-2 hover:bg-[#161718] transition-all">
                                                    <div className="grid grid-cols-[50px_120px_100px_1fr_100px_100px] items-center gap-4">
                                                        
                                                        {/* 1. Rank & Code */}
                                                        <div className="flex flex-col border-r border-gray-700 pr-2">
                                                            <span className="text-[10px] text-gray-500 uppercase">Rank</span>
                                                            <span className="text-xl font-bold text-white leading-none">{p.predicted_pos}</span>
                                                        </div>

                                                        {/* 2. Driver & Team */}
                                                        <div className="flex flex-col min-w-[120px]">
                                                            <span className="text-lg font-black text-white italic leading-tight">{p.driver}</span>
                                                            <span className="text-[10px] text-gray-400 uppercase tracking-tighter">{p.team}</span>
                                                        </div>

                                                        {/* 3. Metrics (Consistency & Speed) */}
                                                        <div className="flex flex-col text-sm font-mono">
                                                            <div className="flex justify-between text-[10px]">
                                                                <span className="text-gray-500">CONSISTENCY</span>
                                                                <span className="text-white">±{p.consistency.toFixed(3)}s</span>
                                                            </div>
                                                            <div className="flex justify-between text-[10px] mt-1">
                                                                <span className="text-gray-500">TOP SPEED</span>
                                                                <span className="text-white font-bold">{p.features?.max_speed || 330} <small className="text-[8px]">KM/H</small></span>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div className="px-4">
                                                            <div className="flex justify-between items-end mb-1">
                                                                <span className="text-[9px] text-gray-500 font-bold uppercase">Perf Index</span>
                                                                <span className="text-xs font-mono text-white">{p.performance_index.toFixed(1)}</span>
                                                            </div>
                                                            <div className="w-full bg-gray-800 h-1.5 rounded-full overflow-hidden">
                                                                <div 
                                                                    className={`h-full transition-all duration-500 ${
                                                                        p.performance_index > 90 ? 'bg-emerald-500' : p.performance_index > 75 ? 'bg-yellow-500' : 'bg-orange-500'
                                                                    }`}
                                                                    style={{ width: `${p.performance_index}%` }}
                                                                />
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div className="text-right">
                                                            <span className="text-[10px] text-emerald-500 block font-bold uppercase">Interval</span>
                                                            <span className="text-sm font-mono text-white">
                                                                {i === 0 ? p.base_pace.toFixed(3) + 's' : `+${intervalGap?.toFixed(3)}s`}
                                                            </span>
                                                        </div>

                                                        {/* 6. Base Pace (Final Column) */}
                                                        <div className="bg-gray-900/50 p-2 rounded text-right min-w-[90px]">
                                                            <span className="text-sm font-mono text-white font-bold tracking-tight">
                                                                {p.base_pace.toFixed(3)}s
                                                            </span>
                                                        </div>
