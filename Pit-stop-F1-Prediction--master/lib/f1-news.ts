...
export async function getLatestUpdates(): Promise<NewsResponse> {
    ...
    // ===== 0. OFFICIAL RSS NEWS (Sky Sports, BBC, Autosport) =====
    try {
        const [bbcFeed, skyFeed, autosportFeed] = await Promise.all([
            parser.parseURL('https://feeds.bbci.co.uk/sport/formula1/rss.xml').catch(() => ({ items: [] })),
            parser.parseURL('https://www.skysports.com/rss/12433').catch(() => ({ items: [] })),
            parser.parseURL('https://www.autosport.com/rss/f1/').catch(() => ({ items: [] }))
        ]);
        ...
        allItems.forEach(item => {
            ...
            // Recovery: Scan content for <img> tags if still empty
            const contentToScan = item.content || item.contentSnippet || '';
            if (!imageUrl && contentToScan) {
                const imgMatch = contentToScan.match(/<img[^>]+src="([^">]+)"/);
                if (imgMatch) imageUrl = imgMatch[1];
            }

            if (imageUrl) {
                // Pro Scaling: Transform low-res thumbs to 1080p/1024 placeholders
                imageUrl = imageUrl
                    .replace('/240/', '/1024/')
                    .replace('/480/', '/1024/')
                    .replace('/small/', '/large/')
                    .replace('?w=140', '?w=1024'); // Sky Sports specific scaling
                
                if (imageUrl.startsWith('http:')) imageUrl = imageUrl.replace('http:', 'https:');
            }
            
            // Pro-Fallback: Thematic assignment to avoid 'empty' cards
            const currentTitle: string = item.title || '';
            const currentSnippet = item.contentSnippet || '';
            const textForImg = (currentTitle + ' ' + currentSnippet).toLowerCase();
            
            if (!imageUrl) {
                if (textForImg.includes('paddock') || textForImg.includes('interview')) {
                    imageUrl = 'https://images.unsplash.com/photo-1510214690353-85f2ea1e0b57?q=80&w=1024'; // Steering wheel/cockpit
                } else if (textForImg.includes('car') || textForImg.includes('wing') || textForImg.includes('tyre')) {
                    imageUrl = 'https://images.unsplash.com/photo-1541410965313-d53b3c16ef17?q=80&w=1024'; // Formula car on track
                } else {
                    imageUrl = 'https://images.unsplash.com/photo-1535165910397-25983e87ae14?q=80&w=1024'; // Asphalt track
                }
            }

            // Smart category detection from title + content
            const text = (currentTitle + ' ' + currentSnippet).toLowerCase();
            ...
            if (/regulat|rule|fia|ban|penalty|steward|cost.?cap|budget|technical.?directive/i.test(text)) {
                category = 'regulation';
            } else if (/transfer|sign|contract|move|join|leave|replac|seat|lineup|line-up|driver.?market|hamilton.?ferrari|newey/i.test(text)) {
                category = 'transfer';
            } else if (/race|grand.?prix|gp|winner|podium|result|finish|qualif|sprint|grid|start|lap|overtake|crash|retire|dnf|safety.?car|p1|p2|p3/i.test(text)) {
                category = 'race';
            } else if (/engine|power.?unit|aero|technical|upgrade|floor|wing|diffuser|sidepod|tyre|tire|brake|suspen|setup|telemetry|carbon|suspension/i.test(text)) {
                category = 'technical';
            }
            
            // Auto-tagging for Breaking news
            const isBreaking = /breaking|confirmed|official.?statement|breaking.?news|exclusive/i.test(text);

            updates.push({
                id: item.guid || item.link || Math.random().toString(),
                title: currentTitle || 'F1 Update',
                summary: currentSnippet.substring(0, 160) + '...',
                category: category,
    ...
    // ===== 0.2 OPTIONAL: WORLD NEWS API INTEGRATION =====
    // To use World News API, add your key to .env and uncomment this block
    /*
    try {
        const apiKey = process.env.WORLD_NEWS_API_KEY;
        if (apiKey) {
            const apiRes = await fetch(`https://api.worldnewsapi.com/search-news?text=formula-1&api-key=${apiKey}`);
            const data = await apiRes.json();
            data.news.forEach((n: any) => {
                updates.push({
                    id: String(n.id),
                    title: n.title,
                    summary: n.text.substring(0, 160),
                    category: 'official',
                    date: n.publish_date,
                    source: n.url.split('/')[2],
                    url: n.url,
                    imageUrl: n.image,
                });
            });
        ...
        if (response.ok) {
            ...
            if (race && race.Results) {
                ...
                updates.push({
                    ...
                    url: `https://www.formula1.com/en/results.html/${race.season}/races.html`,
                ...
                if (fastest) {
                    ...
                    updates.push({
                        ...
                        url: `https://www.formula1.com/en/results.html/${race.season}/fastest-laps.html`,
    ...
    const regulationUpdates: F1NewsItem[] = [
        {
            ...
            imageUrl: 'https://images.unsplash.com/photo-1510214690353-85f2ea1e0b57?q=80&w=1024' // Steering wheel
        },
        {
            id: 'reg-2026-aero',
            ...
            date: '2026-01-20',
            source: 'FIA Technical Regulations',
            imageUrl: 'https://images.unsplash.com/photo-1541410965313-d53b3c16ef17?q=80&w=1024' // Formula car handling
        },
        {
            id: 'reg-2026-weight',
            ...
            imageUrl: 'https://images.unsplash.com/photo-1535165910397-25983e87ae14?q=80&w=1024' // Asphalt
        },
        {
            id: 'reg-2026-chassis',
            ...
            date: '2026-02-05',
            source: 'FIA Technical Regulations',
            imageUrl: 'https://images.unsplash.com/photo-1541410965313-d53b3c16ef17?q=80&w=1024' // Formula car
        },
        {
...
export function getCategoryDetails(category: F1NewsItem['category']): {
    ...
    const info: { [key: string]: { label: string; color: string; bg: string; icon: ElementType } } = {
        ...
        race: { label: 'RACE RESULT', color: '#a0d9f8', bg: 'rgba(160, 217, 248, 0.12)', icon: Trophy },
        technical: { label: 'TECHNICAL', color: '#2293D1', bg: 'rgba(34, 147, 209, 0.12)', icon: Settings },
        breaking: { label: 'BREAKING', color: '#a0d9f8', bg: 'rgba(160, 217, 248, 0.2)', icon: Zap },
